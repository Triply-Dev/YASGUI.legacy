(function(){
	...
	this.Yasgui.Codemirror = {};
	
	this.Yasgui.Codemirror.deleteLines = function(cm) {
	...
	this.Yasgui.Codemirror.commentLines = function(cm) {
		var startLine = cm.getCursor(true).line;
		var endLine = cm.getCursor(false).line;
		var min = Math.min(startLine, endLine);
		var max = Math.max(startLine, endLine);
	...
	this.Yasgui.Codemirror.copyLineUp = function(cm) {
	...
	this.Yasgui.Codemirror.copyLineDown = function(cm) {
		this.Yasgui.Codemirror.copyLineUp(cm);
		//Make sure cursor goes one down (we are copying downwards)
		var cursor = cm.getCursor();
		cursor.line++;
		cm.setCursor(cursor);
	};
	this.Yasgui.Codemirror.doAutoFormat = function(cm) {
	...
	this.Yasgui.Codemirror.indentTab = function(cm) {
	...
	this.Yasgui.Codemirror.unindentTab = function(cm) {
		for (var i = cm.getCursor(true).line; i <= cm.getCursor(false).line; i++) {
			var line = cm.getLine(i);
			if (/^\t/.test(line)) {
				console.log("tab!");
	...
	this.Yasgui.Codemirror.autoComplete = function(cm) {
		if (cm.somethingSelected()) {
			//do nothing
		} else {
	...
	this.Yasgui.Codemirror.prefixHint = function(cm) {
	 ...
	 * @param cm
	 */
	this.Yasgui.Codemirror.appendPrefixIfNeeded = function(cm) {
