(function() {
	...
	Completion.prototype = {
		...
		showWidget : function(data) {
			...
			var startPos = this.cm.getCursor(), startLen = this.cm
					.getLine(startPos.line).length;
			...
			function update() {
				if (isDone())
					return;
				if (completion.options.async)
					completion.getHints(completion.cm, finishUpdate,
							completion.options);
				else
					finishUpdate(completion.getHints(completion.cm,
							completion.options));
			}
			...
			function activity() {
				...
				var pos = completion.cm.getCursor(), line = completion.cm
						.getLine(pos.line);
				if (pos.line != startPos.line
					...
					//disable updating widget. this sometimes causes the widget to draw twice, causing longer
					//loading times.. Not sure what causes it (i.e. what is causing the cursorActivity event)
					//just disable updating the widget for now on cursor activity (not needed anyway I presume)
	...
	function Widget(completion, data) {
		...
		var pos = cm.cursorCoords(options.alignWithWord !== false ? data.from
				: null);
		...
		cm
				.on(
						...
						this.onScroll = function() {
							var curScroll = cm.getScrollInfo(), editor = cm
									.getWrapperElement()
